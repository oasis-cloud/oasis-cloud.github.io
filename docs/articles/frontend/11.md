...
因为组件文档遵循此规范，所以可以很方便的通过脚本进行处理。首先我们要使用 `markdown-it` 对 doc.md 文档进行解析。由此得到 markdown 的 token。通过对 token 进行迭代处理，提取出 Props 相关的表格，并将 table 相关的 token 转换为二维数组。
...
```
面对上面这种代码，通过增则匹配替换出错的机率比较大，而且正则写起来也不容易。然而通过 AST 的方式进行处理更合理，更加稳定。
...
到这里就完成了通过 jscodeshift 向类型文件增加注释的功能。代码细节可以参考NutUI-React 仓库中的 [add-comments-to-dts.js](https://github.com/jdf2e/nutui-react/blob/next/scripts/add-comments-to-dts.js) 文件。
